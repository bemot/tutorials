...
understand the basic dependecies of what we call ‘modules’. The full
interdependiencies and more detailed information can be found in our
model documentation: <https://rse.pik-potsdam.de/doc/magpie/>

Here we want to just shortly draw your attention on the ‘main line’ of
module
interdependencies.
...
<img src="figures/magpie_model.gif" width="100%" style="display: block; margin: auto;" />
  ...
  - Note that this are not one way dependencies. E.g. constrains in land
    availability can have implications on food consumption patterns due
    to rising food prices for different commodities.
...
the `full.gms`. All code chunks are stored within the `modules` folder.
Every module, representing a component of the model, has several
realizations, which are found in the `modules` folder. For the final
model execution, exactly one realization of each module is chosen and
included. The configuration settings that are set in `default.cfg` (or
inside the run scripts) determine the realization entering the
`full.gms`.
...
<img src="figures/module_struc.png" title="structure of any module" alt="structure of any module" width="30%" style="display: block; margin: auto;" />
...
`7_advanced_changeCode.Rmd`). In that sense MAgPIE is easily extentable.
...
Note that not every gms-files is needed in every
realization.
...
represents a seperated part of the model, that could be representated in
a simple or more sophisticated manner, without relying on other modules.
...
interface variables. The is ensured by defined rules for variable and
parameter naming.
...
The prefixes have to be extended in some cases by a second
    letter:
...
In other cases, the prefixes have to be extended by a second
    letter:
