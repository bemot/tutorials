...
#### Start a model run
For a quick test, we simply set the new realization in the file `main.gms` in line 256 (replace `static` by `dynamic`). We can then check if the model compiles correctly with this command evoked from the command line.
```{r, eval=FALSE}
gams main.gms action=C
```

Before we start a test run, we reduce the number of time steps to 3. For this, we change `$setglobal c_timesteps  coup2100` in main.gms in line 224 to `$setglobal c_timesteps  quicktest`.
